...
async function main() {
    ...
    const hexDump: Array<string> = []
    const toUse: Array<string> = []

    for (let i = 0x0; i < 0xFFFFFF; i++) {
        hexDump.push(i.toString(16).padStart(6, '0'))
    }
    ...
    for (let i = 0; i < 100; i++) {
        ...
        if (index == ${i}) {
            color.hexNum = "${hex}";
            color.name = "${removeDiacritics(name)}";
        }
        `;
    }
    ...
    const solidityPath = join(__dirname, `../contracts/GmRenderer.sol`);
